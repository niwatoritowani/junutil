...
jun.plot32 <- function(datax,ylabel1,xlabel1,label1,posi1,posi2) {
    ...
    p1=ggplot(datax, aes(x=R2D2,y=values,fill=GROUP)) +
        ...
        theme(text=element_text(size=8)) +    # default: 7
        scale_fill_manual(values=c("grey","red"),labels=c("HC","CHR")) +    # use default color if commented out
        geom_dotplot(binaxis="y",stackdir="center") +
        stat_summary(fun.ymin=function(x) mean(x), fun.ymax=function(x) mean(x), 
            geom="errorbar", width=0.3) + # add mean as a error bar
...
ppi=300    # 300 is better
png("plot_vol_group_CCC_20160317_300dpi_w3.3h2.5inch.png",width=3.3*ppi, height=2.5*ppi, res=ppi)
p1
dev.off()

ppi=300    # 300 is better
png("plot_vol_group_TH_20160317_300dpi_w3.3h2.5inch.png",width=3.3*ppi, height=2.5*ppi, res=ppi)
options(scipen=2) # default: options(scipen=0)
p3
options(scipen=0) # default
dev.off()
...
ppi=300    # 300 is better
png("plot_vol_group_AMY_20160317_300dpi_w3.3h2.5inch.png",width=3.3*ppi, height=2.5*ppi, res=ppi)
p4
dev.off()
...
# - I could not change shape in addition to fill. 2016/03/17.
# - When I used both fill and shape, the error bars turned to be separated by the subgroup fill-shape. 2016/03/07. 
...
jun.plot32 <- function(datax,ylabel1,xlabel1,label1,posi1,posi2) {
    ...
    ylabel=paste("Relative olumes of",ylabel1)    # combine tests for y label
    p1=ggplot(datax, aes(x=GROUP,y=values,fill=GROUP,shape=ind)) +
        theme_bw() + # change the thema to brack and white
        theme(text=element_text(size=7)) +
        scale_fill_manual(values=c("grey","red"),labels=c("HC","CHR")) +    # use default color if commented out
        geom_dotplot(binaxis="y",stackdir="center") +
